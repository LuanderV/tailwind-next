...
const fileItem = tv({
  slots: {
    ...
    icon: "rounded-full p-2 border-4 border-violet-100 bg-violet-200 text-violet-600 dark:border-zinc-700 dark:bg-zinc-800 dark:border-zinc-700 dark:text-zinc-500",
    deleteButton: "",
  },
...
export function FileItem({ name, size, state }: FileItemProps) {
  ...
  return (
    <div className={container()}>
      ...
      {state === "complete" ? (
        <CheckCircle2 className="h-5 w-5 fill-violet-600 text-white dark:text-white/70" />
      ) : (
